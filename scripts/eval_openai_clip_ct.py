...
def evaluate_category(category, text, class_overview, img_dir, preprocess, top_k):
    ...
    class_metrics = compute_classwise_metrics(true_labels, predicted_labels, idx_to_label)
...
def compute_classwise_metrics(true_labels, pred_labels, mapping):
    ...
    class_metrics = {
        mapping[i]: {'Accuracy':class_wise_acc[i],'Precision': precision[i], 'Recall': recall[i], 'F1 Score': f1[i]}
