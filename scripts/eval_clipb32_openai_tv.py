...
model, _, _ = open_clip.create_model_and_transforms(model_name, pretrained=pretrain_dataset) # quickgelu is used by openai
...
class CustomImageDataset(Dataset):
    def __init__(self, labels, emb_dir):
        """
        Args:
            txt_file (string): Path to the txt file with image names and labels.
            emb_dir (dict): Dictionary with all embeddings.
    ...
    def _create_image_label_pairs(self, labels_dict):
        ...
        for image_name, label in labels_dict.items(): 
            img_labels.append((image_name, self.label_to_idx[label]))
        return img_labels
...
def evaluate_category(category, text, class_overview, emb_dir, top_k):
    # load annotations
    print(category)
    ...
    dataset = CustomImageDataset(labels=label_file, emb_dir=emb_dir)
    ...
    with torch.no_grad():
        for images, labels in tqdm.tqdm(dataloader):
            # load embeddings
            image_features, labels = images.to(device), labels.to(device)
...
def save_convmat(conv_mat, topic, label_overview):
        ...
        plt.savefig(f"conf_mat/openclip_{model_name}_{pretrain_dataset}/tv_{topic}_norm.png", bbox_inches="tight")
        plt.close()
...
emb_dict = torch.load("embeddings/tv_ViT-B-32_openai.torch")
print(len(emb_dict))

for category in text.keys():
    class_metrics, overall_metrics  = evaluate_category(category, text, class_overview, emb_dict, top_k=3)
